...
/** The poster is always drawn at this size, whatever it is displayed at. */
...
export interface ArticlePosterCanvasProps {
  ...
  articleUrl: string;
  /** Transparent PNG frame overlay, served by the consuming app (same origin, or export breaks). */
 ...
 *
 * <p>One renderer for both the preview and the export, and now for both the public site's share
 * sheet and the newsroom's Instagram composer — the image that goes out is the image that was
 * shown. Scale it for display by transforming a wrapper; never by changing these dimensions, or
 * the export stops matching the preview.
 */
export const ArticlePosterCanvas = forwardRef<HTMLDivElement, ArticlePosterCanvasProps>(
  function ArticlePosterCanvas(
    ...
    return (
      ...
      >
        ...
        >
          <span style={{ whiteSpace: "nowrap", padding: "0 4px" }}>{categoryLabel}</span>
        </div>
        ...
        >
          <QRCodeSVG value={articleUrl} size={138} bgColor="#FFFFFF" fgColor="#000000" />
        </div>
