...
export function StoryBlockComposer({
  ...
  return (
    <div className="space-y-4 font-sans">
              ...
              {block.type === "HEADING" && (
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 text-xs font-bold text-slate-900 bg-slate-50 border border-slate-200 rounded-xl p-2.5 focus:outline-none focus:border-red-500 font-serif"
                  />
                </div>
              )}
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 text-xs text-slate-900 bg-slate-50 border border-slate-200 rounded-xl p-2.5 focus:outline-none focus:border-red-500 font-mono"
                    />
                    {onOpenMediaLibrary && (
                      <button
                        type="button"
                        onClick={() => onOpenMediaLibrary(block.id)}
                        className="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs font-bold transition flex items-center gap-1 shadow-2xs font-sans"
                      >
              ...
              {block.type === "PULL_QUOTE" && (
                <div className="space-y-2">
                  <textarea
                    ...
                    value={block.content}
                    onChange={(e) => updateBlock(block.id, { content: e.target.value })}
                    placeholder="Enter impactful pull quote text..."
                    className="w-full text-xs italic font-serif text-slate-900 bg-slate-50 border border-slate-200 rounded-xl p-3 focus:outline-none focus:border-red-500 leading-relaxed resize-none"
                  />
                  <input
                    type="text"
                    value={block.credit || ""}
                    onChange={(e) => updateBlock(block.id, { credit: e.target.value })}
                    placeholder="Quote Attribution / Author Name..."
                    className="w-full text-xs text-slate-900 bg-slate-50 border border-slate-200 rounded-xl p-2 focus:outline-none focus:border-red-500 font-sans"
                  />
                </div>
              )}

              {(block.type === "VIDEO" || block.type === "AUDIO" || block.type === "CALLOUT" || block.type === "TIMELINE" || block.type === "SOURCE") && (
                <div className="space-y-2">
                  <input
                    type="text"
                    value={block.content}
                    onChange={(e) => updateBlock(block.id, { content: e.target.value })}
                    placeholder={`Enter ${block.type.toLowerCase()} details or URL...`}
                    className="w-full text-xs text-slate-900 bg-slate-50 border border-slate-200 rounded-xl p-2.5 focus:outline-none focus:border-red-500 font-sans"
                  />
                </div>
              )}
            </div>
      ...
      {/* Add Block Toolbar */}
      <div className="p-3 border border-slate-200 bg-slate-50/50 rounded-2xl flex flex-wrap items-center justify-between gap-2 text-xs font-mono">
        <span className="text-slate-500 font-bold uppercase text-[10px]">Add Content Block:</span>
        <div className="flex flex-wrap items-center gap-1.5">
          <button
            type="button"
            onClick={() => addBlock("PARAGRAPH")}
            className="px-3 py-1 bg-white hover:bg-slate-100 text-slate-800 border border-slate-200 rounded-xl font-bold transition flex items-center gap-1 shadow-2xs"
          >
            <Type className="h-3 w-3 text-red-600" /> Paragraph
          </button>
          <button
            type="button"
            onClick={() => addBlock("HEADING")}
            className="px-3 py-1 bg-white hover:bg-slate-100 text-slate-800 border border-slate-200 rounded-xl font-bold transition flex items-center gap-1 shadow-2xs"
          >
            <HeadingIcon className="h-3 w-3 text-red-600" /> Heading
          </button>
          <button
            type="button"
            onClick={() => addBlock("IMAGE")}
            className="px-3 py-1 bg-white hover:bg-slate-100 text-slate-800 border border-slate-200 rounded-xl font-bold transition flex items-center gap-1 shadow-2xs"
          >
            <ImageIcon className="h-3 w-3 text-red-600" /> Image
          </button>
          <button
            type="button"
            ...
            className="px-3 py-1 bg-white hover:bg-slate-100 text-slate-800 border border-slate-200 rounded-xl font-bold transition flex items-center gap-1 shadow-2xs"
          >
            <Quote className="h-3 w-3 text-red-600" /> Pull Quote
          </button>
          <button
            type="button"
            ...
            className="px-3 py-1 bg-white hover:bg-slate-100 text-slate-800 border border-slate-200 rounded-xl font-bold transition flex items-center gap-1 shadow-2xs"
          >
            <AlertCircle className="h-3 w-3 text-red-600" /> Callout
          </button>
        </div>
      </div>
    </div>
