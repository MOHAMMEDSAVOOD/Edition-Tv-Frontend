...
export function BreakingTicker() {
  ...
  const items = (mounted && tickers && tickers.length > 0)
  ...
  return (
    <div className="breaking-ticker py-1.5 text-xs font-semibold tracking-wide bg-primary text-black max-w-full overflow-x-hidden" suppressHydrationWarning>
      <div className="container mx-auto max-w-[1200px] px-4 md:px-6 flex items-center gap-3 overflow-hidden" suppressHydrationWarning>
        <span className="flex-none flex items-center gap-1.5 pr-4 border-r border-black/30 uppercase tracking-widest text-[10px] font-mono font-extrabold" suppressHydrationWarning>
          <span className="live-dot inline-block h-2 w-2 rounded-full bg-red-600 animate-ping" />
          Breaking
        </span>

        {(!mounted || isLoading) ? (
          <div className="text-[11px] text-black/70 animate-pulse font-mono" suppressHydrationWarning>
          ...
          <div className="overflow-hidden flex-1 relative" suppressHydrationWarning>
            <div className="ticker-track" suppressHydrationWarning>
              {doubled.map((item, i) => (
                <span key={i} className="inline-flex items-center" suppressHydrationWarning>
